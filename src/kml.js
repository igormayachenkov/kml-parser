...
export class Kml {
    ...
    parseFromString(kmlText) {
      console.log('KML.parseFromString'); // Parse: string => DOM Document
  
      let domParser = new DOMParser();
      if (!domParser) throw String('DOMParser is unsupported');
      let domDocument = domParser.parseFromString(kmlText, "text/xml"); // Verify structure: kml/Document
      ...
      if (root.tagName !== 'Document') throw String('The root tag within <kml> must be <Document>'); // Parce root as a Document
    ...
    deselectAll() {
      if (this.root) this.root.deselectAll();
    }
  
    getSelected() {
      let selected = [];
      if (this.root) this.root.fillSelected(selected);
      return selected;
    }
  ...
  export class Feature {
    ...
    fillSelected(selected) {
      if (this.isSelected) selected.push(this);
    }
  ...
  export class Placemark extends Feature {
    ...
    updateMapDrawing() {
      if (this.geometry) this.geometry.updateMapDrawing(this.kml.map, this.name, this.isSelected);
    }
  
    locate() {
      if (this.kml.map && this.geometry) this.geometry.locate(this.kml.map);
    } // SELECTED STATE
  
  
    setSelected(selected) {
      if (selected && this.kml.options.singleSelection) this.kml.deselectAll();
